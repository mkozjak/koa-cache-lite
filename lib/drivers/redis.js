...
module.exports = class extends events.EventEmitter {
  ...
  remove(keys) {
    if (this.options.debug) console.info('deleting from cache', keys)

    if (!keys)
      this.connection.keys('*').then((results) => this.connection.del(results))
    else if (keys && 'string' === typeof keys)
      this.connection.keys(keys).then((results) => this.connection.del(results))
